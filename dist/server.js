...
server.registerTool('chim_save_api_key', {
    description: 'Stores the CHIM API key in the user config file (~/.config/chim-mcp/config.json).',
...
server.registerTool('chim_list_changes', {
...
server.registerTool('chim_create_change', {
    description: 'Creates a Change notification (POST /api/v1/changes/). Provide the payload documented in the CHIM API guide.',
...
server.registerTool('chim_list_outages', {
...
server.registerTool('chim_create_outage', {
    description: 'Creates an Incident/Outage notification (POST /api/v1/outages/). Provide the payload documented in the CHIM API guide.',
    inputSchema: PayloadSchema
}, async ({ payload, dry_run }) => {
    const parsedPayload = parsePayload(payload);
    if (dry_run) {
...
server.registerTool('chim_list_retros', {
...
function parsePayload(input) {
    if (typeof input === 'string') {
        try {
            ...
            throw new Error(`Unable to parse payload JSON: ${error.message}`);
        }
    }
    return input;
}
